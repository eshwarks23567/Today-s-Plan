...
def plan_node(state: State):
    ...
    p = llm.with_structured_output(Plan).invoke(
        ...
        "2. NEVER invent a venue or showtime. Fill venue/time only if the user named them or "
        "the conversation makes them unambiguous; otherwise leave them null.\n"
...
def drive_browser(url: str, venue: str, time_str: str, seats: int, category: str | None) -> list[str]:
    ...
    try:
        ...
        # ponytail: BMS filters by language/format tabs; if the requested show is on
        # another tab this bails and the human continues — handle tabs if it bites often.
        vkey = venue.split(":")[0].strip() or venue
        anchor = page.get_by_text(vkey, exact=False).first
        anchor.wait_for(timeout=20000)
        anchor.scroll_into_view_if_needed(timeout=8000)
        show = page.locator(
            f"xpath=(//*[contains(normalize-space(),'{vkey}')]/following::*[normalize-space()='{time_str}'])[1]")
        if not show.count():
            show = page.get_by_text(time_str, exact=True).first
        show.click(timeout=8000)
...
def handle(question: str, history: list, listings: str, city: str) -> str:
    try:
        ...
        return booktic.ask_llm(question, listings, history)
